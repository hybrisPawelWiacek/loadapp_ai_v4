...
def test_location_coordinate_types():
    ...
    with pytest.raises(ValidationError):
        Location(latitude="not a number", longitude=13.404954)

    with pytest.raises(ValidationError):
        Location(latitude=52.520008, longitude="not a number")

    # None values
    with pytest.raises(ValidationError):
        Location(latitude=None, longitude=13.404954)

    with pytest.raises(ValidationError):
        Location(latitude=52.520008, longitude=None)
...
def test_location_equality():
    ...
    # Arrange
    loc1 = Location(latitude=52.520008, longitude=13.404954, address="Berlin")
    loc2 = Location(latitude=52.520008, longitude=13.404954, address="Berlin")
    loc3 = Location(latitude=52.520008, longitude=13.404954, address="Different")
    loc4 = Location(latitude=52.0, longitude=13.404954, address="Berlin")

    # Assert
    assert loc1 == loc2  # Same coordinates and address
    assert loc1 != loc3  # Same coordinates, different address
    assert loc1 != loc4  # Different coordinates 